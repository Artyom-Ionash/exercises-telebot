...
def init_db():
    ...
    cursor.execute(
        # Добавим новые значения hp_bonus & dmg_bonus
        """
...
def load_locations(loc_id: int | None = None, loc_name: str | None = None):
    ...
    result: tuple[int, str, str, int, int, int, int] | None = cursor.fetchone()
...
def check_win(passed_locs: list[str]) -> bool:
