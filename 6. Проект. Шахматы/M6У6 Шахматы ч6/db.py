...
def create_game_state(game_id: str, next_black: str, game_state: dict[Position, Figure]):
    """
    ...
    conn = sqlite3.connect("games.db")
    cursor = conn.cursor()
    serialized_game_state = str(game_state).replace("'", '"')
    query = """
        INSERT INTO game_state (game_id, next_black, game_state)
        VALUES (?, ?, ?)
    """
    cursor.execute(query, (game_id, next_black, serialized_game_state))
    conn.commit()
    conn.close()


def update_game_state(game_id: str, next_black: int, game_state: dict[Position, Figure]):
    """
    ...
    query = """
        ...
        WHERE game_id = ?
    """
    cursor.execute(query, (next_black, serialized_game_state, game_id))
    conn.commit()
    conn.close()
...
def user_ids(game_id: str):
    """
    Получает идентификаторы пользователей, участвующих в игре.

    Args:
        game_id (str): Уникальный идентификатор игры, для которой необходимо получить ID пользователей.

    Returns:
        tuple[int, int] | None: Кортеж из идентификаторов пользователей (user_black_id, user_white_id)
        или None, если игра не найдена.
    """
    conn = sqlite3.connect("games.db")
    cursor = conn.cursor()
    cursor.execute(
    ...
    user_ids_res: tuple[int, int] | None = cursor.fetchone()
    conn.close()
    return user_ids_res
